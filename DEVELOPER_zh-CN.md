...
├── compose.yaml             # 生产部署（ktauth + postgres + redis）
├── compose.db.yaml          # 仅数据库（本地开发用）
├── compose.test.yaml        # 测试部署（使用本地构建镜像）
├── .env.example                    # 环境变量模板
...
**Release 产物：**
- `ktauth_{version}.zip`（包含 `compose.yaml`、`00-init.sql`、`.env.example`）
- `install.sh` 安装脚本
...
go test ./... -v                             # 运行所有测试
docker compose -f compose.db.yaml up -d  # 启动开发数据库
