...
本地回环测试，预热 200 次、并发 100（2026-08-31）：

| 场景 | 请求数 | 吞吐量 | P95 |
| :--- | ---: | ---: | ---: |
| `/kt/0` 白名单放行 | 10,000 | 23,245 req/s | 7.51 ms |
| `/kt/0` 随机非白名单 | 10,000 | 18,015 req/s | 8.98 ms |
| `/kt/1` 非白名单拒绝 | 10,000 | 20,468 req/s | 8.27 ms |
| `/kt/0` 滥用转黑名单 | 16,100 | 19,583 req/s | 8.74 ms |

测试环境：Intel Core i9-13900HX、32 GB 内存、Windows 11 Docker（WSL2 后端）。该虚拟化与容器化环境可能带来一定的性能损耗；结果仅描述本次测试，不代表原生 Linux 部署表现。
