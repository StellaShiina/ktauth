...
- **IP 访问管理:**
  ...
  - **CIDR 支持:** 支持 IP 网段（CIDR）的处理与匹配。
- **高级限流策略 (Rate Limiting):**
  - 使用 Redis Lua 脚本和有序集合 (ZSET) 实现了 **滑动窗口算法 (Sliding Window Algorithm)**。
...
2. **环境配置:**
   如果您计划使用邮件服务，请确保已配置必要的环境变量文件（如 `resend.env`, `sendgrid.env`）。
   *(注意: 数据库凭据当前在 `cmd/ktauth/main.go` 中配置。在生产环境中，请务必将其外部化配置。)*
